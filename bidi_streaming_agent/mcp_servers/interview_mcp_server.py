...
mcp = FastMCP("NoraAssistantMCP")


@mcp.tool()
def run_python_code(code: str, stdin_input: str = "") -> str:
    """Execute Python code and return stdout/stderr.
    Use this to validate snippets while reviewing implementation ideas."""
    try:
...
def save_session_notes(user_id: str, notes: str) -> str:
    """Save session notes for a user so you can remember them
    in future sessions. Call this at the END of each session to record:
    - Ideas discussed and how they were scored
    - Competition and market validation findings
    - Code quality or bug-analysis outcomes
    ...
    return f"Session notes saved. You now have {len(history)} session(s) on record for this user."


@mcp.tool()
def recall_session_notes(user_id: str) -> str:
    """Recall previous session notes for a user. Call this at
    the START of each session to remember what you discussed before — problems
    explored, idea outcomes, and follow-up recommendations. Returns all saved
    session notes for this user."""
    user_file = MEMORY_DIR / f"{user_id}.json"
    if not user_file.exists():
        return "No previous session notes found for this user. This appears to be their first session."
...
if __name__ == "__main__":
    print("Starting NoraAssistantMCP server...", file=sys.stderr)
    mcp.run()
